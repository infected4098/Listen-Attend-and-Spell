...
class Speller(nn.Module):
    def __init__(self, output_class_dim, rnn_unit, rnn_hidden_dim, listener_hidden_dim,
        ...
        self.rnn_layer = self.rnn_unit(self.output_class_dim, self.rnn_hidden_dim, num_layers = 2, batch_first = True)
    ...
    def forward_step(self, input_word, decoder_state, listener_feature):
        ...
        return output, hidden_state, context, p_attn
    ...
    def forward(self, listener_tensor, ground_truth = None, teacher_force_rate = 0.95):
        ...
        if teacher_force:  # Training with teacher forcing
            ...
            for step in range(self.max_label_len - 1):
                raw_pred, hidden_state, context, p_attn = self.forward_step(init_word, hidden_state, listener_feature)
